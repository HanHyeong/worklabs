...
- **통계** — 날짜별 총 업무 시간, 기록 수, 집중률 요약 (겹치는 랩 중복 집계 없음)
- **소요시간 변경** — 랩의 시간 표시를 클릭해 소요시간을 즉시 변경
- **겹치는 랩 호버카드** — 같은 시간대에 여러 랩이 있을 경우 마우스 오버로 상세 확인
- **멀티 모니터 지원** — 퀵 등록 창 및 메인 창이 마우스가 있는 모니터 중앙에 표시
...
- 메인 창 오른쪽 상단 **+ 랩 추가** 버튼
- 타임라인 좌측 시간 라벨에 마우스를 올리면 나타나는 **+** 버튼 클릭
- 글로벌 단축키로 퀵 등록 창 호출
...
# 개발 모드 실행 (Vite dev server + Rust hot-reload)
npm run dev
```
